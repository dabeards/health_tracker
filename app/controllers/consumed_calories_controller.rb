class ConsumedCaloriesController < ApplicationController
  ...
  def index
    @consumed_calories = ConsumedCalory.all
  end
  ...
  def new
    @consumed_calory = ConsumedCalory.new
  end
  ...
  def create
    @consumed_calory = ConsumedCalory.new(consumed_calory_params)

    respond_to do |format|
      if @consumed_calory.save
        format.html { redirect_to @consumed_calory, notice: 'Consumed calory was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @consumed_calory.update(consumed_calory_params)
        format.html { redirect_to @consumed_calory, notice: 'Consumed calory was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to consumed_calories_url, notice: 'Consumed calory was successfully destroyed.' }
  ...
  private
    ...
    def set_consumed_calory
      @consumed_calory = ConsumedCalory.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def consumed_calory_params
      params.require(:consumed_calory).permit(:consumed_calories, :calories_date)
    end
end
